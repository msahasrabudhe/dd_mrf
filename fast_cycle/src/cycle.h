...
namespace CycleSolver{
	...
	class Cycle
	{
		public:
			...
			// mmarg[N]:   edge min-marginal (var_id[N-1], var_id[0])
			// mmarg[N+1]: edge min-marginal (var_id[0], var_id[1])
			// ...
		...
		private:
			...
			ValType fast_solver_preprocessing(IdxType last_factor_id, 
	...
	void Cycle::randomCycle(IdxType _N, IdxType K, ValType unary_strength)
	{
		...
		for(IdxType i=0; i<N; i++)
		{
			...
			/* pariwise potential */
			for(IdxType k=0; k<tp[i].size(); k++)
				tp[i][k] = randn(0.0, 1.0);

			/* unary potential */
			if(unary_strength > 0){
				std::vector<ValType> u(card[i], 0);
				for(IdxType k=0; k<u.size(); k++)
					u[k] = randn(0.0, 1.0);
	...
	// and other stuff
	// note: this private function is not optimized for human readability
	ValType Cycle::fast_solver_preprocessing(IdxType last_factor_id, 
			const std::vector<IdxType> & Aidx, 
			const std::vector<IdxType> & Bidx,
			std::vector<ValType> & factor_offset, 
			ValType & global_const, 
